...
on branch `release/2`. Six of eight in-scope examples are fully done.
ShopifyCart has zero errors, deprecations and missing warnings, with seven
A36 witness warnings left. Trello has not been started. See "RIDDL 2.0 Conformance" below.
...
| ShopifyCart | errors/deprecations/missing all zero; **7 completeness left** (A36 witness) |
...
1. **ShopifyCart's last 7 completeness warnings.** All are the A36
   use-case witness pass rejecting the wiring for epic steps of the form
   `step send command X from source UserInterface.ShopperRequests to sink
   ShoppingContext.ShoppingIntake`. There *is* a domain-scoped persistent
   connector between exactly those two ports. Widening its type to
   `ShopperCommand`, an alternation of all eight commands, witnessed one
   step but not the other seven — so the pass appears to match the
   specific message type along the path rather than an alternation
   containing it. Either give each command its own connector, or read
   `passes/.../analysis/UseCaseWitnessPass.scala` and match what it
   actually looks for. Worth checking whether this is a compiler bug
   before contorting the model around it.

2. **Trello.** Untouched, and a from-scratch rebuild by decision — the
   existing 3,011 lines are pre-1.0 and die on a parse error at line 7
   (epic step syntax). Apply the template documented above.

3. **Phase 5 housekeeping**, none of which is started: update CLAUDE.md
   for 2.0, close the three task files per the check-tasks Completion
   Procedure, and delete the two dead Scala test sources.
