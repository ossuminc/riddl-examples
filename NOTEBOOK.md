...
on branch `release/2`. Seven of eight in-scope examples are done. Only Trello remains. See "RIDDL 2.0 Conformance" below.
...
1. **Trello.** Untouched, and a from-scratch rebuild by decision — the
   existing 3,011 lines are pre-1.0 and die on a parse error at line 7
   (epic step syntax). Apply the template documented above.

2. **Phase 5 housekeeping**, none of which is started: update CLAUDE.md
